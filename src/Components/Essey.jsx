...
export class Essey extends Component {
        ...
        window.addEventListener('scroll', () => {
            ...
            const changeContent = (entries) => {
                entries.forEach(entry => {
                    if (this.state.title === 'lorem0') {
                        this.resolveContent();
                    } else {
                        this.resolveContent(true);
                    }
    ...
    resolveContent = (isRemove = false) => {
        const contents = document.getElementById('ContentsContainer');
        if (isRemove) {
            console.log(`${isRemove} should be remove`);
            // TODO: position: absolute;
        } else {
            console.log(`${isRemove} should be not remove`);
            // TODO: position: fixed;
        }
    }
